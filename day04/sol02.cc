...
bool isPartiallyEnclosed(pair<int, int> a, pair<int, int> b) {
...
int main() {
    ...
    while (cin >> input) {
        ...
        if (isFullyEnclosed(pa, pb) || isFullyEnclosed(pb, pa) || isPartiallyEnclosed(pa, pb) || isPartiallyEnclosed(pb, pa)) {
            count += 1;
        }
    }
